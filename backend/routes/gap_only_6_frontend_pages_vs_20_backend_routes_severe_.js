...
// Feature: Only 6 frontend pages vs. 20 backend routes — severe UI gap
  ...
  try {
    ...
    if (!process.env.OPENROUTER_API_KEY) {
      return res.status(503).json({
        ...
        feature: 'gap_only_6_frontend_pages_vs_20_backend_routes_severe_',
      });
    }
    const payload = req.body || {};
    const userPrompt = `Context (JSON):\n${JSON.stringify(payload).slice(0, 4000)}\n\nTask: ${ 'Only 6 frontend pages vs. 20 backend routes — severe UI gap' }\n\nReturn JSON: {"summary": string, "actions": [string], "risks": [string], "confidence": number_0_to_1}.`;
    const system = `You are an expert assistant for the feature: 'Only 6 frontend pages vs. 20 backend routes — severe UI gap'. Respond with strict JSON.`;
    ...
    const out = { feature: 'gap_only_6_frontend_pages_vs_20_backend_routes_severe_', model: aiResp?.model, parsed, raw: content };
    if (dbPool) {
      try {
        await dbPool.query(
          'INSERT INTO gap_features (feature_key, user_id, input_payload, output_payload) VALUES ($1, $2, $3, $4)',
          ['gap_only_6_frontend_pages_vs_20_backend_routes_severe_', req.user?.id || null, JSON.stringify(payload), JSON.stringify(out)]
        );
...
router.get('/health', (req, res) => res.json({ feature: 'gap_only_6_frontend_pages_vs_20_backend_routes_severe_', ok: true, ai_configured: !!process.env.OPENROUTER_API_KEY }));
...
router.get('/history', async (req, res) => {
  ...
  try {
    ...
    const r = await dbPool.query(
      ...
      ['gap_only_6_frontend_pages_vs_20_backend_routes_severe_']
    );
