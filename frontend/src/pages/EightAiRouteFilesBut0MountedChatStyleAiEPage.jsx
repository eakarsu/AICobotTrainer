...
export default function EightAiRouteFilesBut0MountedChatStyleAiEPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/gap-eight-ai-route-files-but-0-mounted-chat-style-ai-e/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Eight ai* route files but 0 mounted /chat-style AI endpoints — feature parity gap</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-eight-ai-route-files-but-0-mounted-chat-style-ai-e/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
