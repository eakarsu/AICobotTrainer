...
export default function RagOverRobotSdkDocsAndSafetyStandardsIso1Page() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/cf-rag-over-robot-sdk-docs-and-safety-standards-iso-1/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>RAG over robot SDK docs and safety standards (ISO 10218, ISO/TS 15066)</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-rag-over-robot-sdk-docs-and-safety-standards-iso-1/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
