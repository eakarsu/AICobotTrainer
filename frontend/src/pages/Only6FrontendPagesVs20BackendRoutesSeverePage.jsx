...
export default function Only6FrontendPagesVs20BackendRoutesSeverePage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/gap-only-6-frontend-pages-vs-20-backend-routes-severe-/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Only 6 frontend pages vs. 20 backend routes — severe UI gap</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/gap-only-6-frontend-pages-vs-20-backend-routes-severe-/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
