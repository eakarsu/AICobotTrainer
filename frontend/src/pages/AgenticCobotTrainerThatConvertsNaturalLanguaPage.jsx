...
export default function AgenticCobotTrainerThatConvertsNaturalLanguaPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const r = await fetch('/api/cf-agentic-cobot-trainer-that-converts-natural-langua/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb', maxWidth: 980 }}>
      <h2 style={{ marginTop: 0 }}>Agentic cobot trainer that converts natural-language tasks into validated motion plans</h2>
      <p style={{ color: '#9ca3af', fontSize: 13 }}>
        Endpoint: <code>/api/cf-agentic-cobot-trainer-that-converts-natural-langua/run</code>. Submit context as text; the backend calls OpenRouter and returns structured JSON.
      </p>
